...
def load_dotenv(start_path: Path) -> None:
    """Đọc tệp .env bằng cách quét ngược lên các thư mục cha (Zero-dependency)"""
    current_dir = start_path.resolve()
    # Tìm kiếm ngược lên tối đa 5 cấp thư mục cha
    for _ in range(5):
        ...
        if dotenv_path.exists():
            for line in dotenv_path.read_text(encoding="utf-8").splitlines():
                ...
                if line and not line.startswith("#") and "=" in line:
                    ...
                    key_strip = key.strip()
                    val_strip = value.strip().strip('"').strip("'")
                    # Nạp biến môi trường nếu chưa có hoặc đang chứa giá trị giữ chỗ
                    if key_strip not in os.environ or "YOUR_GEMINI_API_KEY" in os.environ[key_strip] or not os.environ[key_strip]:
                        os.environ[key_strip] = val_strip
            print(f"[Thông tin] Đã nạp thành công cấu hình từ: {dotenv_path}")
            return
        if current_dir.parent == current_dir:
            break
        current_dir = current_dir.parent
    print("[Cảnh báo] Không tìm thấy tệp .env ở thư mục hiện tại hoặc các thư mục cha!")
...
def main() -> None:
    ...
    # Định vị thư mục session hiện tại để tìm dữ liệu và lưu đầu ra
    script_dir = Path(__file__).resolve().parent
    base_dir = script_dir.parent if script_dir.name == "references" else script_dir
    
    # Tự động quét ngược tìm tệp .env từ thư mục script lên trên
    ...
    # Danh sách các tệp đầu vào của Session 06
    input_files = [
        ("edge-cases-sample.txt", "edge-cases-sample-redacted-gemini.txt"),
        ("prompt-injection-attacks.txt", "prompt-injection-attacks-redacted-gemini.txt"),
    ]
    log_path = base_dir / "outputs/execution-log.csv"

    for in_name, out_name in input_files:
        input_path = base_dir / "synthetic-data" / in_name
        ...
        print(f"\n--- Đang xử lý tệp: {input_path.name} ---")
        ...
        print(f"[Kết quả] Đã xử lý xong tệp tin: {input_path.name}")
