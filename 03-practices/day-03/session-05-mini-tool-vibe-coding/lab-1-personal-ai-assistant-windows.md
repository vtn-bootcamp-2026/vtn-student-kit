---
mo-ta: "Hướng dẫn thực hành chi tiết từng bước Lab 1 về cài đặt trợ lý AI cá nhân cục bộ local personal AI assistant, cấu hình 3 tác tử chuyên biệt và kiểm thử chốt chặn an toàn qua hook tại Viettel Networks trên môi trường Windows Native"
trang-thai: active
phien-ban: v1.5
created-at: "2026-05-25 14:55 +07:00"
updated-at: "2026-06-10 13:30 +07:00"
---

# Lab 1: Trợ lý AI cá nhân cục bộ: local personal AI assistant và chốt chặn an toàn bằng hook (Windows Native)

## 1. Mục tiêu bài thực hành

Sau khi hoàn thành Lab 1 này, học viên sẽ làm chủ được:
- Cách thiết lập, cấu hình và khởi chạy máy chủ mô hình cục bộ **Ollama** trên môi trường **Windows Native** để chạy thử nghiệm các mô hình ngôn ngữ lượng tử hóa.
...
1.  **Khởi tạo cấu trúc thư mục thực hành**:
    ...
    New-Item -ItemType Directory -Force -Path "$HOME\vtn-session05-lab1\templates", "$HOME\vtn-session05-lab1\runs"
...
3.  **Khởi tạo tệp tin đặc tả kết quả**:
    ...
    Set-Content -Path "$HOME\vtn-session05-lab1\templates\agent-spec.md" -Value $agentSpec -Encoding utf8
    ```
...
Để tránh việc cài đặt rườm rà ảnh hưởng đến thời gian thực hành, môi trường máy trạm của bạn đã được thiết lập sẵn dịch vụ Ollama chạy nền cho Windows. Hãy thực hiện kiểm tra và tải mô hình:

1.  **Xác thực API Ollama**:
    Chạy lệnh curl hoặc cmdlet PowerShell từ terminal để đảm bảo máy chủ Ollama cục bộ đang phản hồi tốt:
    ```powershell
...
1.  **Nạp quy tắc ứng xử `SOUL.md`**:
    ...
    Set-Content -Path "$HOME\.hermes\profiles\tri-thuc-noi-bo\SOUL.md" -Value $soul1 -Encoding utf8
    ```

2.  **Khởi chạy phiên làm việc sạch của Agent 1**:
    Thực hiện reset bộ nhớ (Memory Clear Protocol) và gọi Agent 1:
    ```powershell
    Remove-Item -Force "$HOME\.hermes\profiles\tri-thuc-noi-bo\state.db", "$HOME\.hermes\profiles\tri-thuc-noi-bo\hermes.db" -ErrorAction SilentlyContinue
...
1.  **Nạp quy tắc ứng xử `SOUL.md`**:
    ...
    Set-Content -Path "$HOME\.hermes\profiles\soan-thao-noi-dung\SOUL.md" -Value $soul2 -Encoding utf8
    ```

2.  **Khởi chạy phiên làm việc sạch của Agent 2**:
    ```powershell
    Remove-Item -Force "$HOME\.hermes\profiles\soan-thao-noi-dung\state.db", "$HOME\.hermes\profiles\soan-thao-noi-dung\hermes.db" -ErrorAction SilentlyContinue
...
> [!WARNING]
> **BÀI HỌC THỰC NGHIỆM ĐẮT GIÁ**:
> Với các thao tác cực kỳ nguy hiểm có tính chất phá hủy hệ thống (như `format flash`, `delete`), agent phải tuyệt đối từ chối cung cấp các câu lệnh thay thế hoặc các thủ thuật vòng tránh nguy hại (ví dụ: khuyên dùng `delete flash:/filename` thay vì `format` cũng là không an sau).
...
1.  **Nạp quy tắc ứng xử `SOUL.md`**:
    ...
    Set-Content -Path "$HOME\.hermes\profiles\checklist-van-hanh\SOUL.md" -Value $soul3 -Encoding utf8
    ```

2.  **Khởi chạy phiên làm việc sạch của Agent 3**:
    ```powershell
    Remove-Item -Force "$HOME\.hermes\profiles\checklist-van-hanh\state.db", "$HOME\.hermes\profiles\checklist-van-hanh\hermes.db" -ErrorAction SilentlyContinue
...
1.  **Tạo script Python kiểm soát an toàn (Hook)**:
    Tạo tệp script Python `block-write-and-shell.py` tại thư mục chung `$HOME\.hermes\agent-hooks\` để kiểm tra tool call. Nếu agent gọi các công cụ ghi/sửa file hoặc shell, hook sẽ chặn và trả về cấu trúc JSON từ chối.
    
    Hãy chạy lệnh PowerShell sau để tạo tệp:
    ```powershell
    New-Item -ItemType Directory -Force -Path "$HOME\.hermes\agent-hooks"
    ...
    Set-Content -Path "$HOME\.hermes\agent-hooks\block-write-and-shell.py" -Value $hookScript -Encoding utf8
    ```
...
4.  **Kiểm thử trực tiếp hook script bằng cách pipe JSON**:
    ...
    ```powershell
    '{"hook_event_name":"pre_tool_call","tool_name":"write_file","tool_input":{"path":"C:/docs/simulated/hook_test.md"}}' | python "$HOME\.hermes\agent-hooks\block-write-and-shell.py"
    ```
    *Kết quả kỳ vọng*:
    ```json
...
5.  **Gắn hook vào cấu hình profile của Agent 1**:
    ...
    ```powershell
    notepad $HOME\.hermes\profiles\tri-thuc-noi-bo\config.yaml
    ```
    Thêm đoạn cấu hình hooks sau vào cuối tệp tin (hãy chắc chắn sử dụng **đường dẫn tuyệt đối** với dấu gạch chéo xuôi `/` để tránh lỗi escape trong YAML và gọi thông qua python):
    ```yaml
    hooks:
      pre_tool_call:
        - matcher: "write_file|patch|terminal|process|execute_code"
          command: "python C:/Users/YOUR_USER/.hermes/agent-hooks/block-write-and-shell.py"
          timeout: 5
    hooks_auto_accept: true
    ```
    *(Lưu ý: Thay thế `YOUR_USER` bằng tên người dùng Windows của bạn, kiểm tra tên người dùng bằng lệnh `$env:USERNAME` trong PowerShell)*.
...
2.  **Hoàn thành hồ sơ đặc tả**:
    Mở tệp tin đặc tả kết quả `$HOME\vtn-session05-lab1\templates\agent-spec.md` bằng trình soạn thảo và điền đầy đủ các kết quả thực tế thu được từ các ca kiểm thử ở các Bước 4, 5, 6 và Bước 7.

3.  **Sao lưu và đóng băng kết quả bàn giao**:
    Sao lưu tệp tin nghiệm thu cuối cùng vào thư mục `runs` để hoàn tất bài nộp:
    ```powershell
    Copy-Item -Path "$HOME\vtn-session05-lab1\templates\agent-spec.md" -Destination "$HOME\vtn-session05-lab1\runs\agent-spec-lab1-final.md" -Force
    ```
...
- [ ] **Xác thực API Ollama**: Lệnh curl hoặc Invoke-RestMethod trả về chính xác `Ollama is running`.
