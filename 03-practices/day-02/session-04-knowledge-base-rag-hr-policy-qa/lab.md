...
Bạn là **chuyên viên thử nghiệm AI** tại phòng Công nghệ của **VinaTel Network** — một doanh nghiệp viễn thông lớn. Nhiệm vụ: xây dựng **"Kỹ năng Hỏi đáp Chính sách Nhân sự tự trị"** (Agentic RAG Skill).
...
| D. Cross-team testing + đánh giá | 60 phút | D1-D2 | evaluation-report.md + cross-team report |
...
### Part D: Cross-team testing + đánh giá (60 phút)

> [!NOTE]
> **Mỏ neo Slide bài giảng:** Tương ứng với slide **Auto-Evaluation** và **Cross-team Testing**.

---

#### Bước D1: Chạy evaluator và phân tích kết quả (Run Evaluation Pipeline)

Chạy toàn bộ pipeline kiểm thử:
...
📥 **Checkpoint cứu hộ Bước D1:** [checkpoint-step-d1.ipynb](templates/checkpoints/checkpoint-step-d1.ipynb)

---

#### Bước D2: Cross-team testing (Exchange Skill Packages)

Bước kiểm thử độc lập — nhóm khác chạy Skill package của bạn và ngược lại.

**Quy trình:**

1. **Gửi Skill package:** Mỗi nhóm gửi toàn bộ thư mục Skill package (SKILL.md, skill.json, kb/, schemas/, scripts/) cho nhóm khác
2. **Nhận và import:** Nhóm nhận giải nén vào thư mục làm việc riêng
3. **Chạy evaluator:** Nhóm nhận chạy `python scripts/evaluator.py --questions ./synthetic-data/test-questions.csv`
4. **Ghi báo cáo:** Nhóm nhận điền kết quả vào bảng:

| Chỉ số | Kết quả nhóm tác giả | Kết quả nhóm nhận | Nhận xét |
| --- | --- | --- | --- |
| in_scope_accuracy | ... | ... | ... |
...
5. **Phản hồi:** Nhóm nhận gợi ý cải thiện cụ thể (ví dụ: "chunker tạo quá ít chunks cho policy-training.md")

> [!WARNING]
> Nếu nhóm khác không chạy được Skill package của bạn, đó là lỗi triển khai — không phải lỗi kiểm thử. Đảm bảo Skill package có hướng dẫn cài đặt và chạy rõ ràng trong SKILL.md.

**KẾT QUẢ KỲ VỌNG:** Ít nhất 1 nhóm khác chạy thành công Skill package của mình. Báo cáo cross-team hoàn chỉnh.

📸 **Hình ảnh kết quả cuối Bước D2:**

***Cross-team report — SLI so sánh giữa 2 nhóm***
  ![Cross-team report](outputs/screenshots/step-d2-cross-team-report.jpg)

***Skill package cấu trúc hoàn chỉnh — 8/8 files***
  ![Skill package 8 files](outputs/screenshots/step-d2-skill-package-complete.jpg)

📥 **Checkpoint cứu hộ Bước D2:** [checkpoint-step-d2.ipynb](templates/checkpoints/checkpoint-step-d2.ipynb)
...
- [ ] **Cross-team: ≥1 nhóm khác chạy được** Skill package của mình
...
📥 **Checkpoint cứu hộ:** [checkpoint-step-d1.ipynb](templates/checkpoints/checkpoint-step-d1.ipynb)
